...
#define CHECK_SHAPE(x, ...)                                                    \
  PADDLE_CHECK(x.sizes() == torch::IntArrayRef({__VA_ARGS__}),                 \
               #x " must have shape (" #__VA_ARGS__ ")")
#define CHECK_CONTIGUOUS(x)                                                    \
  PADDLE_CHECK(x.is_contiguous(), #x " must be contiguous")
      ...
      PAGEDKV_SWITCH(
          params.page_table && !params.pagedkv_tma, PagedKVNonTMA, [&] {
            PACKGQA_SWITCH(params.pack_gqa, PackGQA_, [&] {
              // Always enable PackGQA for Sm8x or PagedKVNonTMA or Split to
              // reduce compilation
              static constexpr bool PackGQA =
                  PackGQA_ || Arch < 90 || PagedKVNonTMA || Split;
              SOFTCAP_SWITCH(params.softcap > 0.0, Has_softcap, [&] {
                if (!params.is_e4m3) {
                  if (params.is_bf16) {
#ifndef FLASHMASK_V3_DISABLE_HDIM64
                    if (params.d <= 64) {
                      return run_mha_fwd_<Arch, cutlass::bfloat16_t, 64, 64,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM96
                    if (params.d <= 96) {
                      return run_mha_fwd_<Arch, cutlass::bfloat16_t, 96, 96,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
#endif
                    ...
                    if (params.d <= 128) {
                      return run_mha_fwd_<Arch, cutlass::bfloat16_t, 128, 128,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
#endif
                    ...
                    if (params.d <= 192) {
                      if (params.dv <= 128 && Arch == 90) {
                        return run_mha_fwd_<Arch, cutlass::bfloat16_t, 192, 128,
                                            Split, PagedKVNonTMA, Has_softcap,
                                            PackGQA>(params, stream);
                      } else {
                        return run_mha_fwd_<Arch, cutlass::bfloat16_t, 192, 192,
                                            Split, PagedKVNonTMA, Has_softcap,
                                            PackGQA>(params, stream);
                      }
                    }
                    ...
                    if (params.d <= 256) {
                      return run_mha_fwd_<Arch, cutlass::bfloat16_t, 256, 256,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
#endif
...
#ifndef FLASHMASK_V3_DISABLE_HDIM64
                    if (params.d <= 64) {
                      return run_mha_fwd_<Arch, cutlass::half_t, 64, 64, Split,
...
#ifndef FLASHMASK_V3_DISABLE_HDIM96
                    if (params.d <= 96) {
                      return run_mha_fwd_<Arch, cutlass::half_t, 96, 96, Split,
                                          PagedKVNonTMA, Has_softcap, PackGQA>(
                          params, stream);
                    }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM128
                    if (params.d <= 128) {
                      return run_mha_fwd_<Arch, cutlass::half_t, 128, 128,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
#endif
                    ...
                    if (params.d <= 192) {
                      if (params.dv <= 128 && Arch == 90) {
                        return run_mha_fwd_<Arch, cutlass::half_t, 192, 128,
                                            Split, PagedKVNonTMA, Has_softcap,
                                            PackGQA>(params, stream);
                      } else {
                        return run_mha_fwd_<Arch, cutlass::half_t, 192, 192,
                                            Split, PagedKVNonTMA, Has_softcap,
                                            PackGQA>(params, stream);
                      }
                    }
...
#ifndef FLASHMASK_V3_DISABLE_HDIM256
                    if (params.d <= 256) {
                      return run_mha_fwd_<Arch, cutlass::half_t, 256, 256,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
#endif
...
#ifndef FLASHMASK_V3_DISABLE_HDIM64
                  if (params.d <= 64) {
                    return run_mha_fwd_<90, cutlass::float_e4m3_t, 64, 64,
                                        Split, PagedKVNonTMA, Has_softcap,
                                        PackGQA>(params, stream);
                  }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM96
                  if (params.d <= 96) {
                    return run_mha_fwd_<90, cutlass::float_e4m3_t, 96, 96,
                                        Split, PagedKVNonTMA, Has_softcap,
                                        PackGQA>(params, stream);
                  }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM128
                  if (params.d <= 128) {
                    return run_mha_fwd_<90, cutlass::float_e4m3_t, 128, 128,
                                        Split, PagedKVNonTMA, Has_softcap,
                                        PackGQA>(params, stream);
                  }
#endif
                  ...
                  if (params.d <= 192) {
                    if (params.dv <= 128 && Arch == 90) {
                      return run_mha_fwd_<90, cutlass::float_e4m3_t, 192, 128,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    } else {
                      return run_mha_fwd_<90, cutlass::float_e4m3_t, 192, 192,
                                          Split, PagedKVNonTMA, Has_softcap,
                                          PackGQA>(params, stream);
                    }
                  }
...
#ifndef FLASHMASK_V3_DISABLE_HDIM256
                  if (params.d <= 256) {
                    return run_mha_fwd_<90, cutlass::float_e4m3_t, 256, 256,
                                        Split, PagedKVNonTMA, Has_softcap,
                                        PackGQA>(params, stream);
                  }
#endif
...
inline bool get_pagedkv_tma(Flash_fwd_params const &params) {
  if (params.arch < 90 || !params.page_table || params.leftpad_k ||
      params.knew_ptr) {
    return false;
  }
  ...
  auto kBlockMN_kernel_args_sm90 = tile_size_fwd_sm90(
      ...
      false /*paged_kv_non_TMA*/, params.softcap > 0.f, short_seqlen);
  int const kBlockM = std::get<0>(kBlockMN_kernel_args_sm90);
  int const kBlockN = std::get<1>(kBlockMN_kernel_args_sm90);
  // Heuristic: when seqlen_q <= kBlockM, we're not compute bound, and somehow
  // using TMA is slower, at least for MLA.
  return params.page_size % kBlockN == 0 &&
         params.seqlen_q * (params.h / params.h_k) > kBlockM;
}

inline bool get_pack_gqa(Flash_fwd_params const &params) {
  // Always enable PackGQA for Sm8x or PagedKVNonTMA or Split to reduce
  // compilation and binary size. Has little effect on speed.
  if (params.arch < 90 || (params.page_table && !params.pagedkv_tma) ||
      params.num_splits > 1) {
    return true;
  }
  ...
  auto kBlockMN_kernel_args_sm90 = tile_size_fwd_sm90(
      ...
      short_seqlen);
  int const kBlockM = std::get<0>(kBlockMN_kernel_args_sm90);
  return should_pack_gqa(params.cu_seqlens_q || params.seqused_q,
                         params.seqlen_q, params.h / params.h_k, kBlockM);
#endif
}
...
inline int get_num_splits(Flash_fwd_params const &params) {
  ...
  // params.page_table must already be set
  // This needs to match the kernel configs
  bool const varlen = params.cu_seqlens_q || params.cu_seqlens_k ||
  ...
  auto kBlockMN_kernel_args_sm90 = tile_size_fwd_sm90(
      ...
      params.page_table && !params.pagedkv_tma, params.softcap > 0.f,
      short_seqlen);
  // Strictly speaking we need to pass in (varlen && params.num_splits > 1) but
  // num_splits has not been set here. It's OK though because we might just
  // underestimate kBlockN a bit
  auto kBlockMN_kernel_args_sm8x = tile_size_fwd_sm8x(
      params.arch == 86 || params.arch == 89, params.d_rounded,
      params.dv_rounded, params.is_causal, params.is_local,
      params.is_e4m3 ? 1 : 2 /*element_size*/, params.page_table, varlen,
      params.softcap > 0.f, params.knew_ptr);
  int const kBlockM = params.arch >= 90
                          ? std::get<0>(kBlockMN_kernel_args_sm90)
                          : std::get<0>(kBlockMN_kernel_args_sm8x);
  int const kBlockN = params.arch >= 90
                          ? std::get<1>(kBlockMN_kernel_args_sm90)
  ...
  int const seqlen_k_loaded =
      !params.is_local
          ? params.seqlen_k
          : std::max(0, std::min(params.seqlen_k, params.window_size_right +
  ...
  int const size_one_kv_head =
      params.seqlen_k * (params.d + params.dv) * (params.is_e4m3 ? 1 : 2);
  // Always enable PackGQA for Split
  // If varlen, we use dynamic split, so this heuristic just needs to get an
  // upper bound on num_splits. We assume the case where there's 1 long sequence
  // and the rest are short, i.e. pretending that batch = 1.
  int total_mblocks = (params.num_splits_dynamic_ptr ? 1 : params.b) *
                      params.h_k * num_m_blocks;
  return num_splits_heuristic(total_mblocks, params.num_sm, num_n_blocks,
          ...
          if (!params.is_bf16) {
            ...
            if (params.d <= 64) {
              return run_mha_bwd_<Arch, cutlass::half_t, 64, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM96
            if (params.d <= 96) {
...
#ifndef FLASHMASK_V3_DISABLE_HDIM192
            if (params.d <= 192) {
              return run_mha_bwd_<Arch, cutlass::half_t, 192, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM256
            if (params.d <= 256) {
              return run_mha_bwd_<Arch, cutlass::half_t, 256, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#else
...
#ifndef FLASHMASK_V3_DISABLE_HDIM64
            if (params.d <= 64) {
              return run_mha_bwd_<Arch, cutlass::bfloat16_t, 64, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM96
            if (params.d <= 96) {
              return run_mha_bwd_<Arch, cutlass::bfloat16_t, 96, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM128
            if (params.d <= 128) {
              return run_mha_bwd_<Arch, cutlass::bfloat16_t, 128, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM192
            if (params.d <= 192) {
              return run_mha_bwd_<Arch, cutlass::bfloat16_t, 192, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
#ifndef FLASHMASK_V3_DISABLE_HDIM256
            if (params.d <= 256) {
              return run_mha_bwd_<Arch, cutlass::bfloat16_t, 256, Has_softcap,
                                  Is_causal, Deterministic>(params, stream);
            }
#endif
...
Flash_fwd_params *flashmaskv3_create_fwd_params_handle() {
  Flash_fwd_params *params_handle =
      (Flash_fwd_params *)malloc(sizeof(Flash_fwd_params));
  if (params_handle) {
...
Flash_bwd_params *flashmaskv3_create_bwd_params_handle() {
  Flash_bwd_params *params_handle =
      (Flash_bwd_params *)malloc(sizeof(Flash_bwd_params));
  if (params_handle) {
...
Flash_fwd_params *
flashmaskv3_cast_to_fwd_params_handle(Flash_bwd_params *params_handle) {
...
void flashmaskv3_run_mha_fwd_combine(Flash_fwd_params *params_handle,
                                     cudaStream_t stream,
                                     bool enable_pdl = false) {
  run_mha_fwd_combine(*params_handle, stream, enable_pdl);
}

void flashmaskv3_run_mha_fwd(Flash_fwd_params *params_handle,
                             cudaStream_t stream) {
  run_mha_fwd(*params_handle, stream);
}

void flashmaskv3_run_mha_bwd(Flash_bwd_params *params_handle,
                             cudaStream_t stream) {
  // printf("point1\n");
...
#define DEFINE_GETTER_SETTER(type, member)                                     \
  type flashmaskv3_fwd_params_get_##member(                                    \
      const Flash_fwd_params *params_handle) {                                 \
    return params_handle->member;                                              \
  }                                                                            \
  void flashmaskv3_fwd_params_set_##member(Flash_fwd_params *params_handle,    \
                                           type value) {                       \
    params_handle->member = value;                                             \
  }                                                                            \
  type flashmaskv3_bwd_params_get_##member(                                    \
      const Flash_bwd_params *params_handle) {                                 \
    return params_handle->member;                                              \
  }                                                                            \
  void flashmaskv3_bwd_params_set_##member(Flash_bwd_params *params_handle,    \
                                           type value) {                       \
    params_handle->member = value;                                             \
  }
