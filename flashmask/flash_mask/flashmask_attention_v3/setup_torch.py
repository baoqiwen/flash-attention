...
FORCE_BUILD = os.getenv("FLASH_MASK_V3_FORCE_BUILD", "FALSE") == "TRUE"
SKIP_CUDA_BUILD = os.getenv("FLASH_MASK_V3_SKIP_CUDA_BUILD", "FALSE") == "TRUE"
...
DISABLE_FP16 = os.getenv("FLASH_MASK_V3_DISABLE_FP16", "FALSE") == "TRUE"
DISABLE_FP8 = os.getenv("FLASH_MASK_V3_DISABLE_FP8", "FALSE") == "TRUE"
DISABLE_VARLEN = os.getenv("FLASH_MASK_V3_DISABLE_VARLEN", "FALSE") == "TRUE"
DISABLE_CLUSTER = os.getenv("FLASH_MASK_V3_DISABLE_CLUSTER", "FALSE") == "TRUE"
DISABLE_HDIM64 = os.getenv("FLASH_MASK_V3_DISABLE_HDIM64", "FALSE") == "TRUE"
DISABLE_HDIM96 = os.getenv("FLASH_MASK_V3_DISABLE_HDIM96", "FALSE") == "TRUE"
DISABLE_HDIM128 = os.getenv("FLASH_MASK_V3_DISABLE_HDIM128", "FALSE") == "TRUE"
DISABLE_HDIM192 = os.getenv("FLASH_MASK_V3_DISABLE_HDIM192", "FALSE") == "TRUE"
DISABLE_HDIM256 = os.getenv("FLASH_MASK_V3_DISABLE_HDIM256", "FALSE") == "TRUE"
...
def create_build_config_file():
    CONFIG = {
        "build_flags": {
            "FLASHMASK_V3_DISABLE_BACKWARD": DISABLE_BACKWARD,
            "FLASHMASK_V3_DISABLE_SPLIT": DISABLE_SPLIT,
            "FLASHMASK_V3_DISABLE_PAGEDKV": DISABLE_PAGEDKV,
            "FLASHMASK_V3_DISABLE_APPENDKV": DISABLE_APPENDKV,
            "FLASHMASK_V3_DISABLE_LOCAL": DISABLE_LOCAL,
            "FLASHMASK_V3_DISABLE_SOFTCAP": DISABLE_SOFTCAP,
            "FLASHMASK_V3_DISABLE_PACKGQA": DISABLE_PACKGQA,
            "FLASHMASK_V3_DISABLE_FP16": DISABLE_FP16,
            "FLASHMASK_V3_DISABLE_FP8": DISABLE_FP8,
            "FLASHMASK_V3_DISABLE_VARLEN": DISABLE_VARLEN,
            "FLASHMASK_V3_DISABLE_CLUSTER": DISABLE_CLUSTER,
            "FLASHMASK_V3_DISABLE_HDIM64": DISABLE_HDIM64,
            "FLASHMASK_V3_DISABLE_HDIM96": DISABLE_HDIM96,
            "FLASHMASK_V3_DISABLE_HDIM128": DISABLE_HDIM128,
            "FLASHMASK_V3_DISABLE_HDIM192": DISABLE_HDIM192,
            "FLASHMASK_V3_DISABLE_HDIM256": DISABLE_HDIM256,
            "FLASHMASK_V3_DISABLE_SM8x": DISABLE_SM8x,
            "FLASHMASK_V3_DISABLE_SM8X": DISABLE_SM8X,
            "FLASHMASK_V3_ENABLE_VCOLMAJOR": ENABLE_VCOLMAJOR,
            "FLASH_MASK_V3_DISABLE_HDIMDIFF64": DISABLE_HDIMDIFF64,
            "FLASH_MASK_V3_DISABLE_HDIMDIFF192": DISABLE_HDIMDIFF192,
        }
    }
...
# ===== flashmask_v2 feature toggles (对应 CMake 里的 option) =====
# DISABLE_FP16    = check_env_flag("DISABLE_FP16", "ON")
# DISABLE_FP8     = check_env_flag("DISABLE_FP8", "ON")
# DISABLE_HDIM64  = check_env_flag("DISABLE_HDIM64", "OFF")
# DISABLE_HDIM96  = check_env_flag("DISABLE_HDIM96", "ON")
# DISABLE_HDIM128 = check_env_flag("DISABLE_HDIM128", "OFF")
# DISABLE_HDIM192 = check_env_flag("DISABLE_HDIM192", "ON")
# DISABLE_HDIM256 = check_env_flag("DISABLE_HDIM256", "OFF")
# DISABLE_SPLIT   = check_env_flag("DISABLE_SPLIT", "ON")
# DISABLE_PAGEDKV = check_env_flag("DISABLE_PAGEDKV", "ON")
# DISABLE_SOFTCAP = check_env_flag("DISABLE_SOFTCAP", "ON")
# DISABLE_PACKGQA = check_env_flag("DISABLE_PACKGQA", "ON")
# DISABLE_BACKWARD= check_env_flag("DISABLE_BACKWARD", "OFF")
...
def is_offline_build() -> bool:
    ...
    may set `FLASH_MASK_V3_OFFLINE_BUILD` in the build environment to prevent any attempts at downloading
    ...
    """
    return check_env_flag("FLASH_MASK_V3_OFFLINE_BUILD", "")
...
if not SKIP_CUDA_BUILD:
    ...
    if os.path.isdir(instantiations_dir):
        for fname in os.listdir(instantiations_dir):
            fpath = os.path.join(instantiations_dir, fname)
            if os.path.isfile(fpath):
                st = os.stat(fpath)
                with open(fpath, "rb") as f:
                    digest = hashlib.md5(f.read()).digest()
                _file_snapshots[fpath] = (st.st_atime, st.st_mtime, digest)

    result = subprocess.run(
        [sys.executable, "./generate_kernels.py",
         "-o", "./instantiations"],
    ...
    # Restore timestamps for files whose content didn't change
    _restored = 0
    for fpath, (old_atime, old_mtime, old_digest) in _file_snapshots.items():
        if os.path.isfile(fpath):
            with open(fpath, "rb") as f:
                new_digest = hashlib.md5(f.read()).digest()
    ...
    if bare_metal_version >= Version("12.3") and bare_metal_version < Version("12.8"):
        download_and_copy(
            name="nvcc",
    ...
    flash_api_source = "flash_api_torch.cpp"

    sources = (
        [flash_api_source, "flash_api_cuda_torch.cu"]
        + (sources_fwd_sm80 if not DISABLE_SM8x else []) + sources_fwd_sm90
        + (sources_bwd_sm80 if not DISABLE_SM8x else []) + sources_bwd_sm90
    )
    sources = [s for s in sources if "hdim64_512" not in s]
