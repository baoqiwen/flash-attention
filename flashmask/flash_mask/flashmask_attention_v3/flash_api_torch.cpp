...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      void *seqused_q, void *seqused_k, void *softmax_lse_d,
  ...
  TORCH_CHECK(p_dropout < 1.f, "p_dropout must be less than 1.0");
  ...
  params.is_local =
      (window_size_left >= 0 || window_size_right >= 0) && !params.is_causal;
...
void set_params_dgrad(Flash_bwd_params &params,
                      ...
                      at::Tensor &dv, void *cu_seqlens_q_d,
                      ...
                      int window_size_left, int window_size_right,
                      const cudaDeviceProp &dprops, const float softcap = 0.f,
                      bool deterministic = false, int const sm_margin = 0) {
...
void run_mha_fwd_entry(Flash_fwd_params &params, cudaStream_t stream) {
  flashmaskv3_run_mha_fwd(&params, stream);
}

void run_mha_fwd_combine_entry(Flash_fwd_params &params, cudaStream_t stream,
                               bool enable_pdl = false) {
#ifndef FLASHMASK_V3_DISABLE_SPLIT
  flashmaskv3_run_mha_fwd_combine(&params, stream, enable_pdl);
#else
  TORCH_CHECK(false,
              "This flash attention build does not support combine kernels.");
#endif
}

inline bool get_pagedkv_tma_entry(Flash_fwd_params &params) {
  return flashmaskv3_get_pagedkv_tma(&params);
}

inline bool get_pack_gqa_entry(Flash_fwd_params &params) {
  return flashmaskv3_get_pack_gqa(&params);
}

inline int get_num_splits_entry(Flash_fwd_params &params) {
  return flashmaskv3_get_num_splits(&params);
}

inline int get_max_headdim() { return 256; }
...
// h_k: num_heads_k
// d: head_size
std::tuple<at::Tensor, at::Tensor> mha_fwd(
    const at::Tensor
        &q, // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    const at::Tensor
        &k, // (b_k, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k
            // or (num_pages, page_size, h_k, d) if there is page_table.
    const at::Tensor &
        v, // (b_k, s_k, h_k, dv) or (total_k, h_k, dv) if there is cu_seqlens_k
    ...
    const std::optional<at::Tensor>
        &v_new_, // (b, s_k_new, h_k, dv) or (total_k_new, h_k, dv) if there is
                 // cu_seqlens_k_new
    const std::optional<at::Tensor> &q_v_, // (b, s_q, h, dv) or (total_q_new,
                                           // h, dv) if there is cu_seqlens_q
    const std::optional<at::Tensor>
        &out_, // (b, s_q, h, dv) or (total_q, h, dv) if there is cu_seqlens_q
    const std::optional<at::Tensor> &cu_seqlens_q_,     // b+1
    const std::optional<at::Tensor> &cu_seqlens_k_,     // b+1
    const std::optional<at::Tensor> &cu_seqlens_k_new_, // b+1
    const std::optional<at::Tensor>
    ...
    const std::optional<at::Tensor> &leftpad_k_, // b
    const std::optional<at::Tensor>
        &rotary_cos_, // seqlen_ro x (rotary_dim / 2)
    const std::optional<at::Tensor>
        &rotary_sin_, // seqlen_ro x (rotary_dim / 2)
    // std::optional<at::Tensor> seqlens_rotary_, // b
    const std::optional<at::Tensor> &q_descale_, // (b, h_k), not (b, h)
    const std::optional<at::Tensor> &k_descale_, // (b, h_k)
    const std::optional<at::Tensor> &v_descale_, // (b, h_k)
    ...
    // === 新增参数 ===
    const std::optional<at::Tensor>
        &startend_row_indices_,                   // 新增：FlashMask 核心索引
    const std::optional<at::Tensor> &block_mask_, // 新增：BlockMask
    const bool is_rotary_interleaved, // if true, rotary combines indices 0 & 1,
                                      // else indices 0 & rotary_dim / 2
    const std::optional<at::Tensor> &scheduler_metadata_, // (b + 1)
    int64_t num_splits, const std::optional<bool> pack_gqa_,
    const int64_t sm_margin) {

  auto dprops = at::cuda::getCurrentDeviceProperties();
  // bool is_sm8x = dprops->major >= 8;
  // TORCH_CHECK(is_sm8x, "FLASHMASK_V3 only supports Ampere GPUs or newer.");
  // Paddle: const bool is_sm90 = dprops.major == 9 && dprops.minor == 0;
  bool is_sm90 = dprops->major == 9 && dprops->minor == 0;
  TORCH_CHECK(is_sm90, "FlashAttention-3 (FlashMaskV3) only supports Hopper "
                       "GPUs (e.g., H100/H800).");
  ...
  TORCH_CHECK(q_type == at::ScalarType::Half ||
              ...
              "FLASHMASK_V3 only supports fp16, bf16, and fp8_e4m3 data type");
  ...
  int const seqlen_k =
      !is_varlen_k ? (!paged_KV ? k.size(1) : max_num_pages_per_seq * page_size)
                   : max_seqlen_k_.value();
  ...
  TORCH_CHECK(head_size <= max_headdim,
              "FLASHMASK_V3 forward only supports head dimension at most " +
  ...
  if (head_size_v != head_size) {
    ...
    if (head_size <= 64 && head_size_v > 64) {
      std::cout << "[Warning] 使用了备用 Kernel,性能可能受限..." << std::endl;
    }
  ...
  bool const is_flashmask = startend_row_indices_.has_value();
  bool const is_blockmask = block_mask_.has_value();
  ...
  if (seqused_q_.has_value()) {
    ...
    TORCH_CHECK(seqused_q.dtype() == torch::kInt32,
  ...
  if (seqused_k_.has_value()) {
    ...
    TORCH_CHECK(seqused_k.dtype() == torch::kInt32,
  ...
  bool const is_varlen = is_varlen_q || is_varlen_k || seqused_q_.has_value() ||
  ...
  TORCH_CHECK(head_size % alignment == 0,
              "head_size should be a multiple of " + std::to_string(alignment));
  TORCH_CHECK(head_size_v % alignment == 0,
              "head_size_v should be a multiple of " +
  ...
  set_params_fprop(
      ...
      k, v, out, !is_varlen_q ? nullptr : cu_seqlens_q.data_ptr(),
  ...
  if (k_new_.has_value()) { // This needs to be set before get_pagedkv_tma
    ...
    TORCH_CHECK(seqlen_q <= seqlen_k, "If k_new is supplied, it must have "
                                      "seqlen <= the seqlen of the KV cache");
    ...
    int total_k_new =
        !is_varlen_k_new ? batch_size * k_new.size(1) : k_new.size(0);
    if (!is_varlen_k_new) {
  ...
  // Paddle limit: 992 = 32 * 31 (max supported batch in
  // prepare_varlen_num_blocks)
  bool const use_dynamic_split = is_varlen && (params.b <= 992);

  // Temporarily set num_splits_dynamic_ptr to 1 since get_num_splits checks it
  params.num_splits_dynamic_ptr =
      !use_dynamic_split ? nullptr : reinterpret_cast<int *>(1);

  params.pagedkv_tma = get_pagedkv_tma_entry(params);
  params.num_splits =
      num_splits <= 0 ? get_num_splits_entry(params) : num_splits;
  // Always enable PackGQA for Split, and get_pack_gqa requires
  // params.num_splits to decide

  // 使用 FlashMask 特定的 GQA 打包策略函数
  params.pack_gqa =
      pack_gqa_.has_value() ? pack_gqa_.value() : get_pack_gqa_entry(params);

  // This needs to be set after get_num_splits
  at::Tensor tile_count_semaphore; // Contains the semaphore and optionally
                                   // num_splits_dynamic
  // We don't use the persistent scheduler if Split and not Varlen
  bool const scheduler_needs_semaphore =
      params.arch >= 90 ? true // Hopper 上强制开启
                        // ? (((params.is_causal || params.is_local) &&
                        // (params.num_splits == 1)) || is_varlen)
                        : ((params.is_causal && !is_varlen) ||
                           (is_varlen && params.num_splits > 1));
  // params.varlen_sort_batches = !params.is_local; // Use this value for Sort
  // in scheduler template params.head_swizzle = params.is_causal ||
  // params.is_local; // Use this value for LPT in scheduler template
  if (scheduler_needs_semaphore || use_dynamic_split) {
    // int b_rounded = round_multiple(params.b, 4); // for 16 byte alignment of
    // pointers
    int metadata_size =
        int(scheduler_needs_semaphore) + (use_dynamic_split ? params.b : 0);
    ...
    if (scheduler_metadata_.has_value()) {
      ...
      TORCH_CHECK(scheduler_metadata.dtype() == torch::kInt32,
    ...
    int *metadata_ptr = tile_count_semaphore.data_ptr<int>();

    // 1. 设置 Semaphore 指针 (对应 set_tile_count_semaphore)
    // Paddle: scheduler_needs_semaphore ? ptr : nullptr
    params.tile_count_semaphore =
        scheduler_needs_semaphore ? metadata_ptr : nullptr;

    // 2. 设置 Dynamic Split 指针 (对应 set_num_splits_dynamic_ptr)
    // Paddle: use_dynamic_split ? (ptr + 1) : nullptr
    // 关键点：这里显式偏移了 1 个 int 的位置
    params.num_splits_dynamic_ptr =
        use_dynamic_split ? (metadata_ptr + 1) : nullptr;
  }

  if (q_v_.has_value()) {
    TORCH_CHECK(head_size <= 64, "q_v is only supported for head_size <= 64");
    // TORCH_CHECK(head_size_v >= 256, "q_v is only supported for hdim_v >=
    // 256.");
    TORCH_CHECK(q_type == at::ScalarType::Half ||
                    q_type == at::ScalarType::BFloat16,
  ...
  if (rotary_cos_.has_value()) {
    ...
    TORCH_CHECK(rotary_cos.scalar_type() == q_type,
    ...
    TORCH_CHECK(rotary_sin.scalar_type() == q_type,
                "rotary_cos must have the same dtype as query");
  ...
  TORCH_CHECK(params.softcap == 0.0,
  ...
  TORCH_CHECK(!params.pack_gqa || params.arch < 90 ||
  ...
  TORCH_CHECK(!(params.page_table && !params.pagedkv_tma),
  ...
  if (is_flashmask) {
    startend_row_indices = startend_row_indices_.value();
  }
  at::Tensor block_mask;
  if (is_blockmask) {
    block_mask = block_mask_.value();
  }
  at::Tensor flashmask_maxmin, lt_start_row_indices, lt_end_row_indices,
      ut_start_row_indices, ut_end_row_indices;
  if (is_flashmask) {
    TORCH_CHECK(startend_row_indices.dim() == 4,
                "flashmask_attention receive startend_row_indices with dim "
                "[batch_size, num_heads, seq_len, mask_bounds]");
    int64_t mask_bounds = startend_row_indices.size(3);
    TORCH_CHECK(
        mask_bounds == 1 || mask_bounds == 2 || mask_bounds == 4,
        "flashmask_attention startend_row_indices mask_bounds must in [1,2,4]");
    auto flashmask_maxmin_shape = startend_row_indices.sizes().vec();

    int device_id = startend_row_indices.get_device();
    auto dprops = at::cuda::getDeviceProperties(device_id);
    ...
    flashmask_maxmin = at::empty(
        flashmask_maxmin_shape, startend_row_indices.options().dtype(at::kInt));

    lt_start_row_indices = startend_row_indices.slice(3, 0, 1).contiguous();

    if (mask_bounds == 2) {
      if (!is_causal) {
        ut_end_row_indices = startend_row_indices.slice(3, 1, 2).contiguous();
      } else {
        lt_end_row_indices = startend_row_indices.slice(3, 1, 2).contiguous();
      }
    } else if (mask_bounds == 4) {
      ut_end_row_indices = startend_row_indices.slice(3, 3, 4).contiguous();
      lt_end_row_indices = startend_row_indices.slice(3, 1, 2).contiguous();
      ut_start_row_indices = startend_row_indices.slice(3, 2, 3).contiguous();
    }
  }

  if (is_blockmask) {
    TORCH_CHECK(is_flashmask == true,
                "blockmask should be used with flashmask at the same time ");

    TORCH_CHECK(block_mask.dim() == 4,
                "blockmask receive blockmask_indices with dim "
                "[batch_size, num_heads, blocklen_q, blocklen_k]");

    TORCH_CHECK(block_mask.size(2) == (seqlen_q + 127) / 128,
                "blockmask is now only support blockdim_q = 128 ");

    TORCH_CHECK(block_mask.size(3) == (seqlen_k + 127) / 128,
                "blockmask is now only support blockdim_k = 128 ");

    TORCH_CHECK(block_mask.size(1) == startend_row_indices.size(1),
                "blockmask is now only support same "
  ...
  if (is_blockmask) {
    ...
    params.block_mask_ptr = block_mask.data_ptr<int32_t>();
  }

  if (is_flashmask) {
    if (lt_start_row_indices.defined())
      params.lt_start_ptr = lt_start_row_indices.data_ptr<int32_t>();
    else
      params.lt_start_ptr = nullptr;

    if (lt_end_row_indices.defined())
      params.lt_end_ptr = lt_end_row_indices.data_ptr<int32_t>();
    else
      params.lt_end_ptr = nullptr;

    if (ut_start_row_indices.defined())
      params.ut_start_ptr = ut_start_row_indices.data_ptr<int32_t>();
    else
      params.ut_start_ptr = nullptr;

    if (ut_end_row_indices.defined())
      params.ut_end_ptr = ut_end_row_indices.data_ptr<int32_t>();
    else
      params.ut_end_ptr = nullptr;

    if (flashmask_maxmin.defined())
      params.flashmask_maxmin_ptr = flashmask_maxmin.data_ptr<int32_t>();
    else
      params.flashmask_maxmin_ptr = nullptr;

    params.h_flashmask = startend_row_indices.size(1);
    params.h_h_flashmask_ratio = num_heads / startend_row_indices.size(1);
    ...
    params.ut_end_ptr = nullptr;
    params.flashmask_maxmin_ptr = nullptr;
    params.h_flashmask = 0;
    params.h_h_flashmask_ratio = 0;
  }

  if (total_q > 0 && (total_k + params.total_knew) > 0 && num_heads_k > 0) {
    auto stream = at::cuda::getCurrentCUDAStream().stream();
    run_mha_fwd_entry(params, stream);
    if (params.num_splits > 1) {
      ...
      run_mha_fwd_combine_entry(params, stream, true /*enable_pdl*/);
    } else if (scheduler_needs_semaphore &&
               params.skip_scheduler_metadata_computation) {
      // need to zero out the semaphore in this case
      // tile_count_semaphore.index({torch::indexing::Slice(params.tile_count_semaphore_offset,
      // params.tile_count_semaphore_offset + 1)}).zero_();
...
#ifdef FLASHMASK_V3_DISABLE_BACKWARD
void run_mha_bwd_entry(Flash_bwd_params &params, cudaStream_t stream) {
  TORCH_CHECK(false, "Flash-Attention was built with backward disabled");
}
#else
void run_mha_bwd_entry(Flash_bwd_params &params, cudaStream_t stream) {
  flashmaskv3_run_mha_bwd(&params, stream);
}
#endif
...
// h_k: num_heads_k
// d: head_size
std::tuple<at::Tensor, at::Tensor, at::Tensor> mha_bwd(
    const at::Tensor
    ...
    const std::optional<at::Tensor> &
        dv_, // (b, s_k, h_k, dv) or (total_k, h_k, dv) if there is cu_seqlens_k
    const std::optional<at::Tensor> &cu_seqlens_q_, // b+1
    const std::optional<at::Tensor> &cu_seqlens_k_, // b+1
    const std::optional<at::Tensor>
        &seqused_q_, // b. If given, only this many elements of each batch
    ...
    const std::optional<at::Tensor>
        &seqused_k_, // b. If given, only this many elements of each batch
                     // element's keys are used.
    const std::optional<at::Tensor> &startend_row_indices_,
    const std::optional<at::Tensor> &block_mask_,
  ...
  // TORCH_CHECK(!deterministic || head_size_rounded < 256, "Deterministic
  // backward not supported for hdim 256."); Very important that these match the
  // kernel configs
  bool const is_local =
  ...
  if (is_flashmask) {
    TORCH_CHECK(startend_row_indices.dtype() == torch::kInt32,
                "flashmask_attention startend_row_indices must be INT32 type");
    TORCH_CHECK(startend_row_indices.dim() == 4,
                "flashmask_attention receive startend_row_indices with dim "
                "[batch_size, num_heads, seq_len, mask_bounds]");
    int64_t last_dim = startend_row_indices.size(3);
    TORCH_CHECK(last_dim == 1 || last_dim == 2 || last_dim == 4,
                "flashmask_attention startend_row_indices "
                "mask_bounds must in [1,2,4]");

    auto startend_sizes = startend_row_indices.sizes();
    std::vector<int64_t> flashmask_maxmin_shape(startend_sizes.begin(),
                                                startend_sizes.end());
    ...
    flashmask_maxmin =
        at::empty(flashmask_maxmin_shape,
                  startend_row_indices.options().dtype(torch::kInt32));
  ...
  const auto [kBlockM_sm90, kBlockN_sm90] = [&]() -> std::pair<int, int> {
    if (head_size_rounded <= 64) {
      ...
      TORCH_CHECK(false, "head dim is rounded to ", head_size_rounded,
                  ", which is not supported in FlashMask V3 now.");
      return {0, 0};
    }
  }();
  ...
  if (!is_varlen) {
    ...
    softmax_d = torch::empty({batch_size, num_heads, seqlen_q_rounded},
                             opts.dtype(at::kFloat));
    softmax_lse_log2 = torch::empty({batch_size, num_heads, seqlen_q_rounded},
                                    opts.dtype(at::kFloat));
  } else {
    softmax_d = torch::empty({num_heads, total_q_padded_rounded},
                             opts.dtype(at::kFloat));
    softmax_lse_log2 = torch::empty({num_heads, total_q_padded_rounded},
                                    opts.dtype(at::kFloat));
  }
  ...
  if (num_heads_k != num_heads) { // MQA / GQA
    if (!is_varlen) {
      ...
      dk_accum =
          torch::zeros({num_heads_k, total_k_padded_rounded, head_size_rounded},
                       opts.dtype(at::kFloat));
      dv_accum =
          torch::zeros({num_heads_k, total_k_padded_rounded, head_size_rounded},
                       opts.dtype(at::kFloat));
  ...
  set_params_dgrad(
      ...
      softmax_lse.data_ptr(), softmax_d.data_ptr(),
      /*p_dropout=*/0.f, softmax_scale, window_size_left, window_size_right,
  ...
  if (arch >= 90) {
    // Paddle 用 phi::Full(..., 0) -> Torch 用 torch::zeros
  ...
  if (num_heads_k != num_heads && params.deterministic) {
    // TODO: maybe also zero'ed out dk_semaphore and dv_semaphore in the
  ...
  if (is_flashmask) {
    params.lt_start_ptr = lt_start_row_indices.defined()
                              ? lt_start_row_indices.data_ptr<int>()
                              : nullptr;
    params.lt_end_ptr = lt_end_row_indices.defined()
                            ? lt_end_row_indices.data_ptr<int>()
                            : nullptr;
    params.ut_start_ptr = ut_start_row_indices.defined()
                              ? ut_start_row_indices.data_ptr<int>()
                              : nullptr;
    params.ut_end_ptr = ut_end_row_indices.defined()
                            ? ut_end_row_indices.data_ptr<int>()
                            : nullptr;
    params.flashmask_maxmin_ptr =
        flashmask_maxmin.defined() ? flashmask_maxmin.data_ptr<int>() : nullptr;
    params.h_flashmask = startend_row_indices.size(1);
    params.h_h_flashmask_ratio = num_heads / startend_row_indices.size(1);
  } else {
    params.lt_start_ptr = nullptr;
  ...
  if (is_blockmask) {
    params.m_block_dim = 128;
    params.n_block_dim = 128;
    params.block_mask_ptr = block_mask.data_ptr<int>();
  }
...
TORCH_LIBRARY(flashmask, m) {
  m.def("fwd("
        ...
        "int num_splits = 1,"
        "bool? pack_gqa = False,"
        "int sm_margin = 0) -> (Tensor(out!), Tensor)");
  m.def("bwd("
        "Tensor dout,"
        "Tensor q,"
        "Tensor k,"
        ...
        "int sm_margin = 0) -> (Tensor, Tensor, Tensor)");
}
