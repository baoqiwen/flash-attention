...
void run_mha_fwd_combine_entry(Flash_fwd_params &params, cudaStream_t stream, bool enable_pdl=false) {
...
mha_fwd(const at::Tensor &q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
        ...
        const std::optional<at::Tensor> &k_new_,  // (b, s_k_new, h_k, d) or (total_k_new, h_k, d) if there is cu_seqlens_k_new
        const std::optional<at::Tensor> &v_new_,  // (b, s_k_new, h_k, dv) or (total_k_new, h_k, dv) if there is cu_seqlens_k_new
        const std::optional<at::Tensor> &q_v_,  // (b, s_q, h, dv) or (total_q_new, h, dv) if there is cu_seqlens_q
        const std::optional<at::Tensor> &out_,  // (b, s_q, h, dv) or (total_q, h, dv) if there is cu_seqlens_q
        ...
        const std::optional<at::Tensor> &seqused_q_, // b. If given, only this many elements of each batch element's queries and outputs are used.
        ...
        const std::optional<at::Tensor> &page_table_, // (b_k, max_num_pages_per_seq)
        const std::optional<at::Tensor> &kv_batch_idx_, // b. indices to index into the KV cache
        ...
        // === 新增参数 ===
        const std::optional<at::Tensor> &startend_row_indices_, // 新增：FlashMask 核心索引
        ...
        int64_t num_splits,
        const std::optional<bool> pack_gqa_,
    ...
    TORCH_CHECK(is_sm90, "FlashAttention-3 (FlashMaskV3) only supports Hopper GPUs (e.g., H100/H800).");
    ...
    TORCH_CHECK(head_size <= max_headdim, "FLASHMASK_V3 forward only supports head dimension at most " + std::to_string(max_headdim));
    ...
    // Paddle limit: 992 = 32 * 31 (max supported batch in prepare_varlen_num_blocks)
    ...
    params.num_splits = num_splits <= 0 ? get_num_splits_entry(params) : num_splits;
    // Always enable PackGQA for Split, and get_pack_gqa requires params.num_splits to decide

    // 使用 FlashMask 特定的 GQA 打包策略函数
    params.pack_gqa = pack_gqa_.has_value() ? pack_gqa_.value() : get_pack_gqa_entry(params);
    ...
    if (scheduler_needs_semaphore || use_dynamic_split) {
        ...
        int metadata_size = int(scheduler_needs_semaphore) + (use_dynamic_split ? params.b : 0);
        ...
        params.tile_count_semaphore = scheduler_needs_semaphore ? metadata_ptr : nullptr;

        // 2. 设置 Dynamic Split 指针 (对应 set_num_splits_dynamic_ptr)
        // Paddle: use_dynamic_split ? (ptr + 1) : nullptr
        // 关键点：这里显式偏移了 1 个 int 的位置
        params.num_splits_dynamic_ptr = use_dynamic_split ? (metadata_ptr + 1) : nullptr;
    ...
    if (is_flashmask) {
        ...
        TORCH_CHECK(mask_bounds == 1 || mask_bounds == 2 || mask_bounds == 4,"flashmask_attention startend_row_indices mask_bounds must in [1,2,4]");
        ...
        if (is_sm90) {
            ...
            const int num_chunk =
                (flashmask_maxmin_shape[2] + chunk_valid_length - 1) / chunk_valid_length;
        ...
        flashmask_maxmin = at::empty(
            ...
            startend_row_indices.options().dtype(at::kInt)
        );
    ...
    if (is_blockmask) {
        ...
        TORCH_CHECK(
            block_mask.size(2) == (seqlen_q + 127) / 128,
            "blockmask is now only support blockdim_q = 128 "
        );

        TORCH_CHECK(
            block_mask.size(3) == (seqlen_k + 127) / 128,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> mha_bwd(
    ...
    const std::optional<at::Tensor> &dq_,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    const std::optional<at::Tensor> &dk_,   // (b, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k
    const std::optional<at::Tensor> &dv_,   // (b, s_k, h_k, dv) or (total_k, h_k, dv) if there is cu_seqlens_k
    const std::optional<at::Tensor> &cu_seqlens_q_,   // b+1
    const std::optional<at::Tensor> &cu_seqlens_k_,   // b+1
    const std::optional<at::Tensor> &seqused_q_, // b. If given, only this many elements of each batch element's queries and outputs are used.
    const std::optional<at::Tensor> &seqused_k_, // b. If given, only this many elements of each batch element's keys are used.
    ...
    if (is_flashmask) {
        ...
        std::vector<int64_t> flashmask_maxmin_shape(startend_sizes.begin(), startend_sizes.end());

        flashmask_maxmin_shape[2] = ((flashmask_maxmin_shape[2] + 31) / 32 + 3) / 4 * 4;
        flashmask_maxmin_shape[3] = 8;

        flashmask_maxmin = at::empty(flashmask_maxmin_shape, 
    ...
    if (is_flashmask) {
        params.lt_start_ptr = lt_start_row_indices.defined() ? lt_start_row_indices.data_ptr<int>() : nullptr;
        params.lt_end_ptr = lt_end_row_indices.defined() ? lt_end_row_indices.data_ptr<int>() : nullptr;
        params.ut_start_ptr = ut_start_row_indices.defined() ? ut_start_row_indices.data_ptr<int>() : nullptr;
        params.ut_end_ptr = ut_end_row_indices.defined() ? ut_end_row_indices.data_ptr<int>() : nullptr;
        params.flashmask_maxmin_ptr = flashmask_maxmin.defined() ? flashmask_maxmin.data_ptr<int>() : nullptr;
        params.h_flashmask = startend_row_indices.size(1);
        params.h_h_flashmask_ratio = num_heads / startend_row_indices.size(1);
    } else {
        params.lt_start_ptr = nullptr;
    ...
    if (total_q > 0 && total_k > 0 && num_heads_k > 0) {
        ...
        dk.zero_();
        dv.zero_();
        if (softmax_d.defined()) softmax_d.zero_();
    } else if (total_q > 0 && num_heads_k > 0) {
        dq.zero_();
        if (softmax_d.defined()) softmax_d.zero_();
    }
