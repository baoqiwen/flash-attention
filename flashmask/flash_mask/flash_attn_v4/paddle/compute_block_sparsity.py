...
def compute_block_sparsity(
    ...
    Args:
        ...
        device: The device to use (e.g. 'gpu', 'gpu:0', int device id).
        compute_full_blocks: Whether to compute full blocks.
        use_fast_sampling: Whether to use 5-point sampling.

    Returns:
        A tuple of (BlockSparseTensors [CuTe], BlockSparseTensorsPaddle).
    """
    ...
    mask_block_idx = _zeros_on_device([batch_size, num_heads, num_m_blocks, num_n_blocks], device)
    full_block_cnt = (
    ...
    compute_block_sparsity.compile_cache[compile_key](
        blocksparse_tensors_paddle[:4],
        seqlen_q,
        seqlen_k,
        aux_tensors,
    )

    return blocksparse_tensors, blocksparse_tensors_paddle
