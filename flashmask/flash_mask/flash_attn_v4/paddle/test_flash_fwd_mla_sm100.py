...
"""
Paddle-side test & benchmark harness for FlashAttentionMLAForwardSm100.

The kernel class itself lives (framework-agnostic) at:
    flash_mask.flash_attn_v4.flash_fwd_mla_sm100

This module mirrors flash_mask.flash_attn_v4.torch.test_flash_fwd_mla_sm100
but targets paddle tensors.
"""
...
from flash_mask.flash_attn_v4.paddle.cute_dsl_utils import dump_kernel_attributes
from flash_mask.flash_attn_v4.paddle.testing import attention_ref


def _randn(shape, dtype=paddle.bfloat16):
    return paddle.randn(shape, dtype=dtype)


def _empty(shape, dtype=paddle.bfloat16):
...
def test_mla_kernel(
    ...
    if compile_key not in compile_cache:
        ...
        if varlen_q:
            Q = _randn([total_q_dummy, nheads, hdim])
            Qv = _randn([total_q_dummy, nheads, hdimv])
            O = _empty([total_q_dummy, nheads, hdimv])
            lse = _empty([nheads, total_q_dummy], dtype=paddle.float32)
            index_topk = (
                paddle.rand([total_q_dummy, topk_length])
                .argsort(axis=-1)
                .astype(paddle.int32)
            )
            cu_seqlens_q_dummy = paddle.arange(
                0, (batch + 1) * seqlen_q, seqlen_q, dtype=paddle.int32
            )
        else:
            Q = _randn([batch, seqlen_q, nheads, hdim])
            Qv = _randn([batch, seqlen_q, nheads, hdimv])
            O = _empty([batch, seqlen_q, nheads, hdimv])
            lse = _empty([batch, nheads, seqlen_q], dtype=paddle.float32)
            index_topk = (
                paddle.rand([batch, seqlen_q, topk_length])
                .argsort(axis=-1)
                .astype(paddle.int32)
            )

        if varlen_k:
            K = _randn([total_k_dummy, nheads_kv, hdim])
            V = _randn([total_k_dummy, nheads_kv, hdimv])
            cu_seqlens_k_dummy = paddle.arange(
                0, (batch + 1) * seqlen_k, seqlen_k, dtype=paddle.int32
            )
        else:
            K = _randn([batch, seqlen_k, nheads_kv, hdim])
            V = _randn([batch, seqlen_k, nheads_kv, hdimv])
    ...
    if varlen_q:
        ...
        seqlens_q = paddle.randint(1, max_seqlen_q + 1, shape=[batch]).astype(paddle.int32)
        cu_seqlens_q = paddle.zeros([batch + 1], dtype=paddle.int32)
        cu_seqlens_q[1:] = seqlens_q.cumsum(0).astype(paddle.int32)
        total_q = int(cu_seqlens_q[-1].item())
    else:
        seqlens_q = paddle.full([batch], seqlen_q, dtype=paddle.int32)
        total_q = None  # unused

    if varlen_k:
        paddle.seed(seed + 2000)
        # Each batch item must have at least topk_length keys so topk gather is valid.
        min_seqlen_k = topk_length if gather_kv else 1
        seqlens_k = paddle.randint(min_seqlen_k, seqlen_k + 1, shape=[batch]).astype(paddle.int32)
        # When causal, every batch item needs seqlens_k[b] >= seqlens_q[b].
        if is_causal:
            seqlens_k = paddle.maximum(seqlens_k, seqlens_q)
        cu_seqlens_k = paddle.zeros([batch + 1], dtype=paddle.int32)
        cu_seqlens_k[1:] = seqlens_k.cumsum(0).astype(paddle.int32)
        total_k = int(cu_seqlens_k[-1].item())
    else:
        seqlens_k = paddle.full([batch], seqlen_k, dtype=paddle.int32)
        total_k = None  # unused

    paddle.seed(seed)  # restore main seed before drawing actual tensors

    # ---- Allocate Q / Qv / O / lse ----
    if varlen_q:
        Q = _randn([total_q, nheads, hdim])
        Qv = _randn([total_q, nheads, hdimv])
        O = _empty([total_q, nheads, hdimv])
        lse = _empty([nheads, total_q], dtype=paddle.float32)
    else:
        Q = _randn([batch, seqlen_q, nheads, hdim])
        Qv = _randn([batch, seqlen_q, nheads, hdimv])
        O = _empty([batch, seqlen_q, nheads, hdimv])
    ...
    # ---- Allocate K / V ----
    if varlen_k:
        K = _randn([total_k, nheads_kv, hdim])
        V = _randn([total_k, nheads_kv, hdimv])
    else:
        K = _randn([batch, seqlen_k, nheads_kv, hdim])
        V = _randn([batch, seqlen_k, nheads_kv, hdimv])
    ...
    if gather_kv:
        ...
        for b in range(batch):
            sl_q_b = int(seqlens_q[b].item())
            sl_k_b = int(seqlens_k[b].item())
            # Draw topk_length unique indices from [0, sl_k_b) for each query in this batch item.
            topk_b = (
                paddle.rand([sl_q_b, sl_k_b])
                .argsort(axis=-1)[..., :topk_length]
                .astype(paddle.int32)
            )  # (sl_q_b, topk_length), all < sl_k_b
            topk_parts.append(topk_b)

        if varlen_q:
            index_topk = paddle.concat(topk_parts, axis=0)  # (total_q, topk_length)
        else:
            index_topk = paddle.stack(topk_parts, axis=0)  # (batch, seqlen_q, topk_length)
    else:
        index_topk = None
    ...
    for b in range(batch):
        qs = int(cu_seqlens_q[b].item()) if varlen_q else b * seqlen_q
        qe = int(cu_seqlens_q[b + 1].item()) if varlen_q else (b + 1) * seqlen_q
        ks = int(cu_seqlens_k[b].item()) if varlen_k else b * seqlen_k
        ke = int(cu_seqlens_k[b + 1].item()) if varlen_k else (b + 1) * seqlen_k
    ...
    if varlen_q:
        O_ref = paddle.concat(O_ref_list, axis=0)  # (total_q, nheads, hdimv)
        O_pt = paddle.concat(O_pt_list, axis=0)
        lse_ref = paddle.concat(lse_ref_list, axis=-1)  # (nheads, total_q)
        lse_pt = paddle.concat(lse_pt_list, axis=-1)
    else:
        O_ref = paddle.stack(O_ref_list, axis=0)  # (batch, seqlen_q, nheads, hdimv)
        O_pt = paddle.stack(O_pt_list, axis=0)
        lse_ref = paddle.stack(lse_ref_list, axis=0)  # (batch, nheads, seqlen_q)
        lse_pt = paddle.stack(lse_pt_list, axis=0)
    ...
    print(f"Paddle max O diff: {(O_pt - O_ref).abs().max().item()}")
    print(f"Paddle mean O diff: {(O_pt - O_ref).abs().mean().item()}")
...
def timeit(fn, *args, **kwargs):
    """Simple CUDA-synchronized benchmark timer (paddle doesn't ship torch.utils.benchmark)."""
...
def benchmark_mla_kernel(
    ...
    if compile_key not in compile_cache:
        ...
        O = _empty([batch, seqlen_q, nheads, hdimv])
        index_topk = (
            paddle.rand([batch, seqlen_q, topk_length]).argsort(axis=-1).astype(paddle.int32)
        )
