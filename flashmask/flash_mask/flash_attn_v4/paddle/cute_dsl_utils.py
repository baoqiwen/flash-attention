...
# Paddle dtype -> CUTLASS dtype mapping
...
@lru_cache
def get_device_capacity(device=None) -> Tuple[int, int]:
    """Return (major, minor) CUDA compute capability for the given device."""
    if device is None:
        device = 0
    elif hasattr(device, "index"):
        device = device.index
    try:
        cap = paddle.device.cuda.get_device_capability(device)
        return (cap[0], cap[1])
    except Exception:
...
def cute_compile_patched(*args, **kwargs):
    """A patched version of cute.compile that dumps the SASS to a file if CUTE_CUBIN_PATH is set."""
...
def to_cute_aux_tensor(t, enable_tvm_ffi=True):
    """Convert paddle tensor to cute tensor for TVM FFI, tailored to FlexAttention aux tensors.
...
def get_broadcast_dims(tensor: paddle.Tensor) -> Tuple[bool, ...]:
    ...
    """
    # paddle uses .strides property (not .stride() method like torch)
    return tuple(s == 0 for s in tensor.strides)
